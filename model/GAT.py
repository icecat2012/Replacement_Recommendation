...
class EdgeLinear(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, bias: bool = True,
        ...
        if in_channels > 0:
            self.weight = Parameter(torch.Tensor(out_channels, in_channels))#.type(torch.double))
        else:
        ...
        if bias:
            self.bias = Parameter(torch.Tensor(out_channels))#.type(torch.double))
        else:
    ...
    def __deepcopy__(self, memo):
        out = EdgeLinear(self.in_channels, self.out_channels, self.bias
...
class GATv2Conv(MessagePassing):
    ...
    def __init__(
        ...
        fill_value: Union[float, Tensor, str] = 1., #'mean',
        bias: bool = True,
        ...
        **kwargs,
        ...
        if edge_dim is not None:
            self.lin_edge = EdgeLinear(edge_dim, heads * out_channels, bias=False, weight_initializer='glorot') #Linear
        else:
            self.lin_edge = None
